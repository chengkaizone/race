...
import static race.cheng.service.Constant.KZBJBJ;
import static race.cheng.service.Constant.X_SPAN;
import static race.cheng.view.MyGLSurfaceView.kzbjwllb;
import static race.cheng.view.MyGLSurfaceView.kzbjyylb;
...
public class KZBJForControl {
	...
	public boolean state = false;// false表示可被碰撞，true表示被撞后飞行中，否则不可碰撞。
	float x;// 摆放的初始位置
	float y;
	float z;
	...
	public void drawSelf(GL10 gl) {
		...
		if (!state) {// 原始状态绘制
			...
			if (currentY > -40) {// 如果已经飞行到地面以下，就不再绘制
	...
	public void checkColl(float carXTemp, float carZTemp, float carAlphaTemp) {
		...
		if (carRow == row && carCol == col) {// 如果大家在同一个格子里，进行严格的碰撞检测KZBJBJ
			double disP2 = (bPointX - x) * (bPointX - x) + (bPointZ - z)
					* (bPointZ - z);
			if (disP2 <= KZBJBJ[id]) {// 碰撞了
