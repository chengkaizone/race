 ...
 * 
 */
public class DrawTrafficLights extends Shape {
	// 立方体长、宽、高
	...
	public static boolean flag = true;// 线程启动标志位
	Cube lightBoard;
	DrawBall light;
	Texture face;
	DrawCylinder pole;
	...
	public DrawTrafficLights(float scale) {
		...
		pole = new DrawCylinder(scale * CYLINDER_HEIGHT, scale * CIRCLE_RADIUS,
				scale * DEGREESPAN, COL);
		poleh = new DrawCylinder(scale * CYLINDER_HEIGHT * 0.2f, scale
				* CIRCLE_RADIUS * 0.2f, scale * DEGREESPAN, COL);
	...
	public void drawSelf(GL10 gl, int texId, int number) {
		...
		gl.glPushMatrix();
		gl.glTranslatef(scale * RADIUS, scale * CYLINDER_HEIGHT - scale
				* CYLINDER_HEIGHT * 0.06f, 0);
		poleh.drawSelf(gl, texId);
		gl.glPopMatrix();

		gl.glPushMatrix();
		gl.glTranslatef(scale * RADIUS, scale * CYLINDER_HEIGHT - scale
				* CYLINDER_HEIGHT * 0.12f, 0);
		poleh.drawSelf(gl, texId);
		gl.glPopMatrix();

		gl.glPushMatrix();
		gl.glTranslatef(scale * RADIUS + scale * CYLINDER_HEIGHT * 0.2f / 2
				+ scale * LENGTH, scale * CYLINDER_HEIGHT - scale
				* CYLINDER_HEIGHT * 0.1f, 0);
		...
		gl.glTranslatef(scale * RADIUS + scale * CYLINDER_HEIGHT * 0.2f / 2
				+ scale * LENGTH, scale * CYLINDER_HEIGHT - scale
				* CYLINDER_HEIGHT * 0.1f, scale * WIDTH);
		...
		if (count % 3 == 2)// 绿灯
		{
			...
			gl.glTranslatef(scale * RADIUS + scale * CYLINDER_HEIGHT * 0.2f / 2
					+ 5 * RADIUS * scale, scale * CYLINDER_HEIGHT - scale
					* CYLINDER_HEIGHT * 0.1f, scale * WIDTH + 0.1f - RADIUS
					* scale / 2);
		...
		if (count % 3 == 0)// 红灯
		{
			...
			gl.glTranslatef(scale * RADIUS + scale * CYLINDER_HEIGHT * 0.2f / 2
					+ RADIUS * scale, scale * CYLINDER_HEIGHT - scale
					* CYLINDER_HEIGHT * 0.1f, scale * WIDTH + 0.1f - RADIUS
					* scale / 2);
			gl.glRotatef(90, 1, 0, 0);
			initLight(gl, 1, 1, 1, 1);// 开灯
		...
		if (count % 3 == 1)// 黄灯
		{
			...
			gl.glTranslatef(scale * RADIUS + scale * CYLINDER_HEIGHT * 0.2f / 2
					+ 3 * RADIUS * scale, scale * CYLINDER_HEIGHT - scale
					* CYLINDER_HEIGHT * 0.1f, scale * WIDTH + 0.1f - RADIUS
					* scale / 2);
			gl.glRotatef(90, 1, 0, 0);
			initLight(gl, 1, 1, 1, 1);// 开灯
	...
	private class Texture {
		...
		public Texture(float scale) {
			vCount = 6;
			float[] verteices = { -scale * LENGTH, scale * HEIGHT, 0,
					-scale * LENGTH, -scale * HEIGHT, 0, scale * LENGTH,
					-scale * HEIGHT, 0,

					-scale * LENGTH, scale * HEIGHT, 0, scale * LENGTH,
					-scale * HEIGHT, 0, scale * LENGTH, scale * HEIGHT, 0 };
			...
			float[] textureCoors = { 0, 0.75f, 0, 1, 0.75f, 1, 0, 0.75f, 0.75f,
					1, 0.75f, 0.75f };
	...
	private void initLight(GL10 gl, float r, float g, float b, float a) {
		...
		// 反射光设置
		float[] specularParams = { 0f * r, 0f * g, 0f * b, 1.0f * a };// 光参数
																		// RGBA
