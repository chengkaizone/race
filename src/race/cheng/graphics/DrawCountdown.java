...
public class DrawCountdown extends Shape {
	...
	public static float Countdown_Z_Offset = -20;// 倒计时牌起始位置

	public static int PictureFlag = 3;// 绘制倒计时板的标志位
	public static boolean flag = true;// 线程控制标志位，默认为true

	Countdown daojishi;// 倒计时板
	public static ThreadGo go;// 运动绘制线程
		...
		public void run() {
			while (flag) {
				...
				if (Countdown_Z_Offset > -11f) {
					...
					if (RaceActivity.soundFlag == true && PictureFlag > 0)// 播放交通灯声音
					{
						MyGLSurfaceView.activity.playSound(8, 0);// 交通灯声音
				...
				if (PictureFlag < 0)// 当倒计时完后，结束线程
				{
					...
					RaceActivity.inGame = true;// 表示当前状态为在游戏中
					RaceActivity.keyFlag = true;// 设置场景可控

					if (RaceActivity.soundFlag == true
							&& RaceActivity.inGame == true)// 当在游戏中，并且音效为可开启状态时，开启声音
					{
						RaceActivity.mpBack.stop();
						RaceActivity.mpBack = MediaPlayer.create(
								MyGLSurfaceView.activity, R.raw.backsound);
						RaceActivity.mpBack.setLooping(true);// 循环播放
						RaceActivity.mpBack.start();// 背景音播放
					}
				}
				try {
					Thread.sleep(60);
