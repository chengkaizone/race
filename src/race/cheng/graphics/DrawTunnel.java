 ...
 * 
 */
public class DrawTunnel extends Shape {
	// 立方体长、宽、高
	...
	private class Cube {
		...
		public Cube(float scale, float LENGTH, float WIDTH, float HEIGHT) {
			...
			float[] verteices = {
					...
					// 顶面
					-scale * LENGTH, HEIGHT, -WIDTH, -scale * LENGTH, HEIGHT,
					WIDTH, scale * LENGTH, HEIGHT, WIDTH,
		...
		public Texture(float scale, float LENGTH, float WIDTH) {
			...
			for (int i = 0; i < COL; i++) {
				for (int j = 0; j < COLUMN; j++) {
					...
					float x3 = -scale * LENGTH + 2 * (i + 1) * scale * LENGTH
							/ COL;
					float y3 = 0;
					float z3 = -WIDTH + 2 * j * WIDTH / COLUMN;

					float x4 = -scale * LENGTH + 2 * (i + 1) * scale * LENGTH
							/ COL;
					float y4 = 0;
