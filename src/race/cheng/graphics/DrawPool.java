...
import static race.cheng.service.Constant.CyArray;
...
public class DrawPool {
	...
	public DrawPool(float partSize) {
		...
		{
			water[i] = new DrawWater(Math.PI * 2 * i / water.length// 水面起始波动角度
			);
		}
		...
		public DrawPoolLand(short[][] yArray, int rows, int cols) {
			...
			for (int j = 0; j < rows; j++)// 循环行
			{
				for (int i = 0; i < cols; i++)// 循环列
				{
					...
					alf.add((yArray[j + 1][i + 1] - 255) * LAND_HIGHEST
							/ 255.0f);
					alf.add(zsz + UNIT_SIZE);
				}
			}
		...
		public DrawWater(double startAngle) {
			...
			for (int j = 0; j < rows; j++) {
				for (int i = 0; i < cols; i++) {
					// 计算当前格子左上侧点坐标
					float zsx = -LOCAL_UNIT_SIZE * cols / 2 + i
							* LOCAL_UNIT_SIZE;
					float zsz = -LOCAL_UNIT_SIZE * rows / 2 + j
							* LOCAL_UNIT_SIZE;
					float zsy = (float) Math.sin(startAngle + i * angleSpan)
							* height;
					// 计算右边一列的y坐标
					float zsyp = (float) Math.sin(startAngle + (i + 1)
							* angleSpan)
							* height;
