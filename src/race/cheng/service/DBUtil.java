...
public class DBUtil {
	...
	public static void insert(double time) {
		...
		sb.append("-");
		sb.append((date.getMonth() > 8) ? (date.getMonth() + 1) : "0"
				+ (date.getMonth() + 1));
		sb.append("-");
		sb.append((date.getDate() > 9) ? (date.getDate()) : ("0" + date
				.getDate()));
		sb.append(" ");
		sb.append((date.getHours() > 9) ? (date.getHours()) : ("0" + date
				.getHours()));
		sb.append(":");
		sb.append((date.getMinutes() > 9) ? (date.getMinutes()) : ("0" + date
				.getMinutes()));
		sb.append(":");
		sb.append((date.getSeconds() > 9) ? (date.getSeconds()) : ("0" + date
				.getSeconds()));

		Log.d("datetemp", sb.toString() + "");
		try {
			createOrOpenDatabase();
			String sql = "insert into history values(" + time + ",'"
					+ sb.toString() + "')";
			Log.d("sql", sql + "");
			sld.execSQL(sql);
			closeDatabase();
	...
	public static ArrayList<String[]> getResult() {
		...
		try {
			...
			while (cur.moveToNext()) {
				...
				ta[0] = ((time / 60 > 9) ? time / 60 : "0" + (time / 60)) + ":"
